...
| UI components | `src/components/`, `src/components/extensions/` | txt2img / img2img / Dictionary / Tags / Video / Upscale / Models / Tools の画面 |
...
- トップレベルタブは `txt2img / img2img / Dictionary / Tags / Video / Upscale / Models / Tools`。小さい補助機能は既存タブ内の折りたたみパネルやモーダルに入れる。Prompt大辞典は検索、挿入、履歴/タグ/Civitai連携、source status確認を横断する独立面なのでDictionaryタブで扱う。
