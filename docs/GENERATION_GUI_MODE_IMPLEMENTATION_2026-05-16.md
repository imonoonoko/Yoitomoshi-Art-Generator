...
> 追記: この記録は `作る / 整える / 高度` モード導入時点のもの。実利用で切替負荷が高かったため、後続でモードタブを廃止し、`基本設定` / `Prompt補助` / `拡張設定` を同じ生成画面にまとめる構成へ更新した。現在の正は [`GENERATION_SECTION_UI_UPDATE_2026-05-16.md`](GENERATION_SECTION_UI_UPDATE_2026-05-16.md)。
