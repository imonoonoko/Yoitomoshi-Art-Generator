...
1. GitHubページの `Code` を押す。
2. `Download ZIP` を選ぶ。
3. ZIPを任意のフォルダへ展開する。
4. 展開したフォルダ内の `Yoitomoshi.bat` を使う。

推奨配置:

```text
C:\Yoitomoshi-Art-Generator
```

日本語パスでも動作する想定ですが、初回公開向けには短い英数字パスの方がトラブルを切り分けやすいです。
...
```text
<Yoitomoshi-Art-Generator>\runtime\forge
```
