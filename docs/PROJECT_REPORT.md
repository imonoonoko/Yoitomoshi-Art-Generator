...
> **位置づけ**: このプロジェクトは **配布目的ではなく、ユーザ個人のゲーム開発における
> AI アート生成環境の自前最適化** のためのもの。よってインストーラ生成・コード署名・
> クロスプラットフォーム対応・自動アップデート等の配布関連作業は **対象外**。
> 開発機(Windows / RTX 4060 Ti 8GB)で快適に動くこと、ゲーム素材生成のワークフローに
> フィットすることを優先する。
>
...
> **結論**: 当プロジェクトは個人のゲーム開発用環境であり、外部 API 課金が継続的に発生する
...
- **既存機能で代替可能**: ♻ "feedback as input" + プロンプト書き換えループは
  「自然言語編集」と完全には別物だが、ゲーム素材を量産する用途では実質ほぼ同等の
  反復編集ができる
...
ゲーム素材生成では「キャラ LoRA + 服装 LoRA + 画風 LoRA」を **複数同時適用** することが
