...
完全個人利用を目的に、自分が使いやすい Stable Diffusion 利用ツールとして作っているフロントエンド。Stable Diffusion WebUI Forge を裏で起動して、独自の React/Electron UI から生成・素材管理・Civitai 連携をまとめて行う。GitHubには、将来の自分が再セットアップや再利用をしやすいようにソースを置く。
...
| 現在の優先順位と運用方針を確認したい | [`docs/ROADMAP.md`](docs/ROADMAP.md) |
...
上部タブは `txt2img / img2img / Video / Upscale / Models / Tools` です。日常の画像生成で触る項目は `txt2img` / `img2img` の左カラムにまとめ、動画生成、拡大、モデル管理、保守ツールは独立した作業面に分けています。

| 上部タブ | 役割 |
|---|---|
| **txt2img** | テキストから画像を生成する標準画面 |
| **img2img** | 入力画像を元に再生成 / Inpaint / AIキャラ追加を行う画面 |
| **Video** | Forge AnimateDiff / FramePack外部連携の短尺動画導線 |
| **Upscale** | Tile ControlNet / Ultimate Upscale などの高解像度化 |
| **Models** | モデル / LoRA のサムネ、Civitai情報、お気に入り、メモ管理 |
| **Tools** | モデル検査、変換、マージ、Tagger、起動診断などの保守作業 |
...
| カテゴリ | 機能 |
|---|---|
| 生成 | txt2img / img2img / Video / Upscale / 入力画像 D&D + Ctrl+V / バッチ |
...
- 主要依存: Electron / electron-vite / Vite 7 / React 19 / Tailwind / Radix UI / Zustand / js-yaml / lucide-react
...
- **完全個人利用向け**: 自分の制作環境を再現・保守するためのツール。GitHubはソース保管と再利用のために使い、エンドユーザー向けの一般配布は目的にしない
- **個人運用向けの一区切り**: Forge代表経路、package / dist、インストーラsmoke、active interruptは確認済み。コード署名、更新メタデータ、正式アイコン承認は、将来本当に配布が必要になった時だけ検討する
- **NSFW フィルタは Civitai 検索のみ**: 生成内容そのもののモデレーションはしない(Forge / モデル側の責任)
- **PNGメタデータ**: A1111 / Forge 形式 (`parameters` チャンク / EXIF UserComment) のみ対応
